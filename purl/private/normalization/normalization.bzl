"""Utils to normalize [purl](https://github.com/package-url/purl-spec)s."""

load("//private/normalization:alpm.bzl", "normalize_alpm")
load("//private/normalization:bitbucket.bzl", "normalize_bitbucket")
load("//private/normalization:composer.bzl", "normalize_composer")
load("//private/normalization:github.bzl", "normalize_github")
load("//private/normalization:hackage.bzl", "normalize_hackage")
load("//private/normalization:huggingface.bzl", "normalize_huggingface")
load("//private/normalization:mlflow.bzl", "normalize_mlflow")
load("//private/normalization:pub.bzl", "normalize_pub")
load("//private/normalization:pypi.bzl", "normalize_pypi")

visibility([
    "//private",
])

_normalizers = {
    "alpm": normalize_alpm,
    "bitbucket": normalize_bitbucket,
    "composer": normalize_composer,
    "github": normalize_github,
    "hackage": normalize_hackage,
    "huggingface": normalize_huggingface,
    "mlflow": normalize_mlflow,
    "pub": normalize_pub,
    "pypi": normalize_pypi,
}

def normalize(
        *,
        type = None,
        namespace = None,
        name = None,
        version = None,
        qualifiers = {},
        subpath = None):
    if not type:
        return None, "Mandatory property 'type' not set"

    components = {
        "type": type.lower(),
        "namespace": namespace,
        "name": name,
        "version": version,
        "qualifiers": qualifiers,
        "subpath": subpath,
    }

    normalizer = _normalizers.get(components["type"])
    if normalizer:
        components = normalizer(components)

    purl = struct(
        type = components["type"],
        namespace = [segment for segment in components["namespace"].split("/") if segment] if components["namespace"] else None,
        name = components["name"],
        version = components["version"],
        qualifiers = components["qualifiers"],
        subpath = [segment for segment in components["subpath"].split("/") if segment] if components["subpath"] else None,
    )
    return purl, None
